...
Run the `tinyapp-reviewer` agent over the current diff before this app ships.
...
2. Invoke the `tinyapp-reviewer` subagent. Hand it a brief prompt naming the scope ("review the diff between main and HEAD" or "review the staged changes" or "review the whole tree") and let it produce its findings.
